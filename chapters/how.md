...
例として、筆者が[実際に行ったFirefoxのウィンドウの不具合の報告](https://bugzilla.mozilla.org/show_bug.cgi?id=1521692)の場合を考えてみます。出発点は、以下のようなメモでした。
...
この段階では、手順を*自分で「細かすぎる」と思うくらいに細かく書くように意識する*のがよいです。というのも、*報告に不慣れな人ほど、荒く・情報が不足した報告を書いてしまいがち*だからです。入れすぎた情報を後で削るのは容易ですが、不足した情報に後から気付くのは難しいです。最終的な報告の形に馬止上げるまでは、基本的には*情報は常に多めに盛り込むようにする*ことを心がけるとよいでしょう。
...
（図：コミットグラフの中の3コミットほどを指して、そこがregression windowで、それより前は正常、それより後は異常ということを図示）

regression windowが特定されていると、「どの変更が原因で壊れたのか」の調査がその範囲の中だけで済むため、修正に必要な労力が大幅に軽減されます。迅速な修正には欠かせない情報と言えます。

### コラム：「regression point」ではない理由
...
（図：regression windowを単一のコミットに特定できないパターン）
...
```
1. 自分が期待通りの結果を得られたことを確認した最後のリビジョンと、自分が期待通りの結果を得られなかったことを確認した最初のリビジョンを選ぶ。
↓
2. コミットグラフ上で両者のちょうど真ん中にあるリビジョンを取得する。
↓
3. ソフトウェアを実際に動かして、期待通りの結果を得られているかどうかを確かめる。
↓                        ↓
期待通りの結果を得られた  ↓
↓                        ↓
4a. そのリビジョンを「期待通りの結果を得られたことを確認した最後のリビジョン」とする。
↓                        ↓
5a. その一つ後のリビジョンは「期待通りの結果を得られなかったことを確認した最初のリビジョン」か？
↓       ↓               ↓
Yes      No               ↓
↓       ↓               ↓
終了    2に戻る           ↓
            ...
            期待通りの結果を得られなかった
                          ↓
            4b. そのリビジョンを「期待通りの結果を得られなかったことを確認した最初のリビジョン」とする。
                          ↓
            5b. その一つ前のリビジョンは「期待通りの結果を得られたことを確認した最後のリビジョン」か？
                          ↓       ↓
                          Yes      No
                          ↓       ↓
                         終了    2に戻る
...
この調査の仕方は広く使われていて、ツールの側にもこれを支援する仕組みがあります。例えば、Gitには`git bisect`というサブコマンドがあり、これを使うと、ユーザーは「Gitがcheckoutしてくれたリビジョンの動作を試して、結果が期待通りかどうかを答える」という操作を繰り返すだけで、Gitが自動的に二分探索を進めてくれます。同様の機能がMercurialにも`hg bisect`というサブコマンドとして実装されています。
...
[^mozregression-usage]: mozregression自体の使い方については、https://www.clear-code.com/blog/2018/7/18.html に筆者による日本語での解説があります。
