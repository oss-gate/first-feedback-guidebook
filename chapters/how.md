...
報告の基本の3要素のうち、労力的に最も大きなウェイトを占めるのが*再現手順*です。実際に、ソフトウェア開発において不具合の修正に要する時間のほとんどは、発生条件の特定に費やされていると言っても過言ではないでしょう。不具合の報告の場合、*それが修正に繋がるかどうかは、「確実に現象が再現する手順」が書かれているかどうかでほぼ決まります*。

しかし、再現手順といっても、何をどう書けばよいのでしょうか？

思い出して下さい、フィードバックするネタの見つけ方の話で「使っている途中で遭遇した些細なつまずきを見落とさないことが大切だ」と述べましたよね。「自分がどんな場面で、どんな操作をして、何をしようとして、どうつまずいたのか」の経緯を箇条書きの形で細かく書きだしてみてください。それが再現手順の叩き台となります。

例として、筆者が実際にFirefoxに対して行った「ポップアップウィンドウのウィンドウコントロール[^window-control]が消えてしまう」という[問題](https://bugzilla.mozilla.org/show_bug.cgi?id=1521688)の場合を考えてみます。出発点は、以下のようなメモでした。

[^window-control]: Windowsではウィンドウの右上にある「最小化」「最大化」「閉じる」の3つのボタンのこと。

* 検証環境上でテスト用ページのポップアップウィンドウを開くボタンをクリックしたら、ポップアップウィンドウにウィンドウコントロールが表示されなかった。
...
* ウィンドウコントロールを伴ってウィンドウが開かれる。

実際の結果：

* ウィンドウコントロールが無いウィンドウが開かれる。実際の画面は以下の通り。
...
[^environment-in-str]: 実際に、筆者が行った報告では1と2は再現手順の中には含めず、見出しを分けて後段で「現象が再現する環境（OSとそのバージョン）」「現象が再現するFirefoxのバージョン」として別途記載しました。この例では説明のためにあえて、環境に関する情報も基本の3要素の中に収める書き方にしています。

最初のたった一行のメモから、どうやってこの報告を作るのか。その過程を詳しく見ていくことにします。
...
いわゆる「クリーンな環境」「まっさらな環境」で再現させる手順をまとめます。設定を一切変更していない、ソフトウェアを初めてインストールし、初めて起動した直後の状態がこれにあたります。
