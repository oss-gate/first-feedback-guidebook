...
クラウドが一般化した今の時代は、このような環境を用意するのも簡単になりました。Microsoft AzureやAmazon EC2のようなクラウドサービスでインスタンスを新規に作成すれば、それで済むからです。コストをかけたくない場合は、[VirtualBox](https://www.virtualbox.org/)や[VMware Workstation Player](https://www.vmware.com/jp/products/workstation-player.html)などを使って自分で仮想マシンを構築するとよいでしょう。ただし、問題の原因がハードウェアやそれに近い部分にあるようだと、仮想環境では駄目で、物理的なハードウェアから用意しなくてはらないという場合もあります。

逆に言うと、以下のように「まっさらでない状態」「現象発生時と異なるソフトウェアやバージョン」を使って現象の再現を真っ先に試みるのは失敗の元です。
...
ある程度の経験を積んだ人であれば、それまでの経験に基づいて「OSは別でもそのソフトウェアのバージョンだけ一致していれば十分」のような判断ができる場合もあります。ただ、そのような判断は非常に難しいものですし、経験者でもアテが外れることはよくあります。また、*違う環境でも似た問題が再現するが、調べてみたら表面的には似ていても問題の内容は全く別だった*、という「他人の空似」に遭遇してしまうこともあります。こうなってしまうと、報告をする上では余計な回り道にしかなりません。

前述の例のケースでは、筆者は以下のような環境を用意しました。

* VirtualBox上の仮想マシン上のWindows 7
* Firefox ESR60.1

元々問題が発生していたのはWindows Server 2008なのに、用意した環境がWindows 7というのは、ここまでの話と矛盾するように見えます。が、これは以下のような理由に基づきます。

* Windows Serverはデスクトップ環境に関しては対応するバージョンのクライアント用Windowsとほぼ同等である。
* Windows版のFirefoxのブラウザウィンドウにおけるウィンドウコントロールはFirefox自体が独自に提供しており、Windowsの標準的なアプリケーションの物は使われていない、ということがすでに分かっている。

つまり、調査する側にある程度の知識があるからできているという特殊なケースにあたります。皆さんはなるべくこのような横着をせず、現象の再現手順を特定するときには*実行環境やソフトウェアのバージョンを揃え、余計なソフトウェアは入っておらず、ユーザー設定も存在しない、という状態から始めることを原則としましょう*。
