...
## コラム：「Regression Window」を特定しよう

不具合にはいくつかの類型的なパターンがありますが、その中でも特によく見られる物に「*regression*」（日本語では「リグレッション」や「後退バグ」）があります。これは、「それまでは正常に動いていた機能が、新しいバージョンでは動かなくなってしまった」という種類の不具合のことを指します。
