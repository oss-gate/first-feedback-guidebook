...
コマンドラインインターフェース（CLI）のソフトウェアでは、システムの情報を出力するオプションが用意されていたり、バージョン情報の一部としてシステムの情報が出力されるようになっていたりする場合があります。以下に例をいくつか挙げます。
...
こういった方法を採れるかどうか自体不明なときは、OSSプロジェクトに不具合を報告する際に「必要な情報の収集方法を教えて下さい」と質問してみましょう。場合によっては、「このコマンドを実行した結果を貼り付けて下さい」といったアドバイスをもらえるかもしれません。

環境情報の収集用の仕組みが用意されていない場合には、以下のことをなるべく細かく調べて列挙してみましょう。
...
* Windowsの場合、ワークグループ名は「WORKGROUP」（既定）のように英数字のみだったか、「検証　組織」のように日本語の文字を含んだワークグループ名か。（ワークグループ名に日本語の文字が含まれると問題になることがあるため）
* Windowsの場合、ドメインに参加しているか、していないか。
* Windows Serverの場合、ドメインコントローラか、ただのドメイン参加か。
* 現象に遭遇したソフトウェアのバージョンやエディション。
* ソフトウェアは64bit用か、32bit用か。（Windowsでは32bit用のソフトウェアを64bit環境でも動かせるため）
* 拡張機能やプラグイン機構を持つソフトウェアの場合、導入済みのそれらのソフトウェアの一覧と、それぞれのバージョン。
* システム全体に影響する設定やソフトウェアを使っている場合、それらの一覧と、ソフトウェアそれぞれのバージョン。（デスクトップテーマや配色の変更、キーボードのキー割り当ての変更、マウスジェスチャやウィルス対策ソフトといった常駐型ユーティリティなど）
* マルチディスプレイか、シングルディスプレイか。
  * マルチディスプレイなら、どのディスプレイでも再現するのか、特定のディスプレイでだけ再現するのか。
* Hi-DPI環境なのか、そうでないのか。

冒頭の事例では、当初は単に「Windows」としていましたが、実際には「Windows Server 2008」環境で発生した不具合でした。そのため、実際の報告では「Windows Server 2008」と明記しました。

ところで、ソフトウェアのバージョンを伝える際に単に「最新版」という言い方をする人は少なくないですが、この言葉は非常に注意を要します。*単に「最新版」と言った場合、実際には複数の対象が該当し得る*からです。いくつか例を紹介しましょう。
...
このように「最新版」という言い方は曖昧なので、報告では必ず「バージョンESR68.4.1」のように*具体的な番号を明記する*ようにしましょう。分からない場合は、どうすればバージョン番号を調べられるか、不具合の報告時に開発者に質問しましょう。

冒頭の例では、現象が発生したのは「Firefox ESR60.1」でしたが、より新しいバージョンの「ESR60.4」でも再現する状況でした。そのため、最終的な報告には「ESR60.4で再現する」とだけ書くことにしました。

なお、後述の手順で再現条件を特定できたら、その現象がそのソフトウェアの*「最新版」でも同じ手順で再現するかどうか*を調べ直すのがおすすめです。もしあなたが使っていた版が少し古い版で、最新版では現象が再現しないのであれば、その問題はすでに修正されている可能性があるからです。
...
これらの場所にユーザー設定のフォルダーがあれば、それをzip形式などに圧縮して開発者に提供することで、開発者の手元で詳細な調査を実施できます。

ただ、ユーザー設定の中には住所や氏名、パスワードといった機密情報が含まれる場合がありますので、信用できない開発者に対してユーザー情報を丸ごと引き渡すのは危険です。どのファイルに何が含まれているかを調べ上げた上で、安全と言える情報だけに限定して提供したり、後述のまっさらな環境で再現条件を整えた状態のユーザー情報のみを提供するようにしたり、といった工夫をする必要があるでしょう。

なお、筆者が業務で行っている法人向けのOSSのサポートのように、契約関係の中で秘密が保たれることを保証できている場合は、機密情報も含めてユーザー情報を丸ごと引き渡す選択を取りやすいです。別の言い方をすると、そのようなことをしても大丈夫だという安心や、秘密を守った上で原因究明を迅速に行ってくれるといったサービスを、お金を出して買うのが「OSSの有償サポート」ということになります。
...
CLIのソフトウェアの場合、*端末のシェル上で実行したコマンド列やその出力結果を、ありのままに記録した上で、コピーしてメールやチケットに貼り付ける*のがお薦めです。たとえば、筆者が作成した[Bashスクリプト製Twitterクライアント](https://github.com/piroor/tweet.sh)での「複数行の発言を投稿できない」という問題の報告であれば、以下の要領です。
...
* Webページの場合、どのページでも問題が起こるのか、特定のページでだけ問題が起こるのか。
...
冒頭の例では、現象について最初は「ポップアップウィンドウを閉じられない」としか認識していませんでした。これをより詳しく書いていくと、以下のようになります。

2. Windows 2008を起動して、ログインする。
3. Firefox ESR60.1をインストールする。
4. デスクトップ上のFirefoxのアイコンをダブルクリックして、Firefoxを起動する。
5. Firefoxのロケーションバーに`http://*******.***/`（※編註：実際にはちゃんとしたURLを使用）と入力し、Enterキーを押して、Webページを開く。
...
このように細かく言葉で説明するのと同様に効果的な方法として、*スクリーンショット（操作画面のキャプチャ画像）*や*スクリーンキャスト（操作の様子のキャプチャ動画）*があります。
...
スクリーンショットやスクリーンキャストの保存の仕方が分からない場合は、*PCの画面をスマホで撮影・録画してアップロードする*方法もあります。

画像や動画を使った説明にはもう1つ、*再現条件として説明しそびれていた情報も一緒に伝わる場合がある*、という利点があります。

たとえば、画像を見れば「Windowsのテーマとしてクラシックテーマを使用している」「macOSの配色をDarkにしている」といったことは一目瞭然です。しかし、ユーザー自身がその状態で日常的に長く使用していると、設定を変えたこと自体を完全に忘れていて、そうでない状態で問題の機能を使ったことがなかった、ということもあり得ます。そうなると、もしその設定が現象の再現に必須の条件だったとしても、*暗黙の再現条件*として、言葉だけでの説明からは抜け落ちてしまい、調査が難航する原因となってしまいます。
