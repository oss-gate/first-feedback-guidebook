...
また、手順の中で実際のWebサイトなどの*外部リソースに依存する部分*があれば、静的なローカルファイルなどで置き換えられるかどうかも検討してみます。たとえば「このページで現象が再現する」という例をURLで示したとして、そのURLに開発者がアクセスできない可能性があります。
...
再現条件を特定できたら、その現象がそのソフトウェアの*「最新版」でも同じ手順で再現するかどうか*を調べましょう。もしあなたが使っていた版が少し古い版で、最新版では現象が再現しないのであれば、その現象は不具合としてすでに修正された可能性があります。
...
ESR60.4という、しばらく更新が継続する調査ポート版の最新版で現象がまだ修正されていなかったことから、これは報告する意義のある問題だったと言えます。
...
自動テストによるCI（継続的インテグレーションテスト）が運用されており、テストが失敗するコミットに対しては自動的にエラーが通知されるというような仕組みがあれば、理想的には後退バグが見過ごされたままにならないはずです。しかし、実際には規模の大きなソフトウェアや複雑なソフトウェアでは、後退バグがCIをすり抜けてしまうことが度々あります。
