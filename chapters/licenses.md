...
[^osd-and-fsd]: FSFは1996年時点で、第1から第3までの自由を含む「自由なソフトウェア」という概念を公表していましたが、これはFSFの活動趣意書（ http://www.gnu.org/bulletins/bull1.txt ）などで「FSFの名前に含まれる自由という言葉の意味は～」とさりげなく語られているに留まっていたようです。Debianプロジェクトがこの概念を条文形式に整理する形で1997年に作成したのがDFSGで、オープンソースの定義はDFSGをほぼそのまま引き写した内容となっています。その後、第1から第3の自由を定義文として参照しやすい形式にして、FSFが改めて「自由なソフトウェアの定義」という文書を公表し、さらにそこに1999年に「第0の自由」が追加された、という成立順になっています。「自由なソフトウェアの定義」の文書としての明確化や、第0の自由の明記は、オープンソース運動の広まりの影に自由なソフトウェアが隠れてしまうことを懸念したFSFが、立ち位置の明確化のために行った物です。

### オープンソースと自由なソフトウェアの違い

前述の4つの自由をすべて備えたソフトウェアのことを、FSFは「自由なソフトウェア」と呼んでいます。定義の内容から、ほとんどのOSSは自由なソフトウェアの条件を満たします[^oss-but-non-free]し、自由なソフトウェアの多くはOSSであると言えます[^free-but-non-oss]。
