...
第1と第3[^fsd-numbering]を実現するためには、ソフトウェアのソースコードが必要です。そのため、これらの条件から演繹的に「*ソースコードが提供されること*」という条件も導かれます。

[^fsd-numbering]: フリーソフトウェアの定義 https://www.gnu.org/philosophy/free-sw.html.en は当初第1から第3までの自由を定義していたものの、後から追加された項目が最後ではなく最初の位置に挿入されたことから、番号付けとしては第1から第4ではなく、第0から第3となっています。

条文には「オープンソースの定義」と似通った部分があると感じる人もいるでしょう。これは、オープンソースの定義の元になっている「Debian Free Software Guideline（DFSG）」という文書が、自由なソフトウェアの定義を参考に書かれたためです[^fsd-and-dfsg]。

[^fsd-and-dfsg]: 自由なソフトウェアの定義は1996年当時すでに第1から第3までの自由を定めており、その内容を参考にする形でDFSGが1997年に書かれました。OSIの結成はその翌年の1998年です。なお、自由なソフトウェアの定義に第0の自由が明記されたのは1999年のことですが、これがオープンソースの定義で「使用の自由（個人・グループ・利用分野に対する差別の禁止）」が明文化されていたことに影響を受けてなのかどうかは、定かではありません。
...
この2つを比較したとき、自由なソフトウェアという考え方は、*短期的なメリットよりは中期的・長期的なメリットのほうに重点を置く*もので、より社会運動的な性質が強いと言えるでしょう。

### 自由の意義

ここまでの本書の語り口と照らし合わせて気が付いた方もいらっしゃるかもしれませんが、実のところ筆者は、「オープンソース」よりも「自由なソフトウェア」のほうを推進する[^oss-or-free]立場と言えます。

[^oss-or-free]: 筆者は今のところ、自分が関わるソフトウェアについて「オープンソースにするか、自由なソフトウェアにするか」を選べる場面では「自由なソフトウェア」のほうをなるべく選ぶようにしつつも、すでに他の人によって非コピーレフトなライセンスでOSSとして公開されている物について、敢えてコピーレフトにしていくよう求めるほどではない、という立場を取っています。
