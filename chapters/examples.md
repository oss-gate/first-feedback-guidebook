...
ここまでの解説の中でも実際に行われたフィードバックの例をいくつか紹介してきましたが、参考として、他にもよいフィードバック・悪いフィードバックの例をご紹介します。
...
組み込み機器向けにカスタマイズしたLinuxディストリビューションを作成するためのツールセットの一種であるYoctoでは、そのLinuxディストリビューションに組み込めるパッケージが多数公開されており、C言語のコンパイラであるclangをそこに組み込むための設定ファイルやスクリプトを提供しているのが、meta-clangというプロジェクトです。畑ケさんがとある組み込みボード[^embedded-board]用にFirefoxをビルドしようとして、Firefoxのビルドに必要なmeta-clangを構成に入れたところ、ビルド結果が想定通りにならず、Firefoxパッケージをビルドできないという状況に遭遇しました。
...
最終的にプルリクエストを出すところにまで至っていますが、元々の報告の時点では、畑ケさんはそこまでのことをするつもりはありませんでした。プロジェクトオーナーの側でより洗練された対策を講じてもらえるだろうと期待していたところ、「パッチを作ってもらえませんか？」と逆に依頼されたことで、腰を上げプルリクエストの作成に至りました。このように、フィードバックは「自分にできることをまずはやる」所から始まりつつ、「できないと思っていたことに挑戦する」機会にもなるということの実例とも言えます。

この報告はプロジェクトに用意されているイシューテンプレート（新しい報告をする際に、空欄を埋めるだけで適切な形式を整えられるように用意されたテンプレート）に基づいて書かれていているため、見出しの立て方が本書で紹介しているパターンとは若干異なっています。しかしながら、「To Reproduce」には再現手順と実際の結果が、「Expected behavior」には期待される結果が書かれており、含まれている情報自体は本書で紹介しているパターンと変わりありません。

フィードバックする前の予備調査の時点で、畑ケさんはこの作者の人が、ここで使おうとしている「MULTILIBS」という機能を使っていないようだということを把握していました。そのため、再現手順や環境の作り方をより細かく具体的に書き、MULTILIBSのことを知らない人でも容易に現象を確認できるようにする、ということを意識したそうです。

文中に登場している「glitch」という単語は、辞書では「故障」「誤動作」「異常」といった意味と出ますが、語源は「slip（滑る）」や「slide（ずらす）」と同じで、ニュアンスとしては「本来の正常な状態からずれてしまっている」という状態を表すそうです。インデントのずれや画像の位置ずれなどにも使える表現ということで、覚えておくとよいでしょう。

ところで、以下の文のおかしいところに皆さんは気が付かれましたか？

> We should handle library directory glitch it llvm-config with LLVM_LIBDIR_SUFFIX.

実はこの文の「it」は誤記で、「in」が正しいです。熟練者でもこのようなミスタイプが残ったまま報告してしまうことがあると思うと、皆さんも、英語の間違いを過度に恐れる必要はないのだなと勇気づけられるのではないでしょうか。
