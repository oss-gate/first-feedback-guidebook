...
2つ目は、Webページ上でマウスカーソル（ポインタ）をかざした位置の語句を辞書で引いて結果をポップアップ表示するというGoogle Chrome/Firefox用の拡張機能「Mouse Dictionary」に対して、筆者が行った連続した一連のフィードバックです。これは「模範的」という意味での「よいフィードバック」ではなく、気負わずカジュアルに行うフィードバックの例として見てください。
...
このソフトウェアは初回起動時に初期設定が必要で、初めて使ったときには初期化手順の案内が表示されます。そのメッセージの内容がFirefoxでの実際のUIに整合していないため、初期化に手間取りました。それをきっかけに行ったフィードバックとなります。プロジェクトオーナーの方が日本語圏の方だということが分かっていたので、最初から日本語で報告してしまいました。
