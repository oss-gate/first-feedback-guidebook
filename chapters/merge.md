...
* 省略可能な要素を省略するかどうか。
  ...
  - `if`での条件分岐で実行する内容が1つだけのとき、波括弧を使うのかどうか？（あるいは、後置if[^following-if]を使うのか？）
* インデントの仕方。
  - ハードタブ[^hard-tab]なのか、ソフトタブ[^soft-tab]なのか？
  - インデントの深さはスペース2文字分なのか、4文字分なのか、8文字分なのか？
* 仮引数の書き方。
  - いくつかの言語では関数の定義時に、引数の既定の値の定義や、名前付きのパラメータとして受け取る書き方などができるが、そのような書き方をするのかどうか？
  - 書き方によっては新しめの実行環境を要求することになる[^newer-environment]が、そのプロジェクトはそのような新しめの書き方を許容しているか？
