...
問題を再定義して、プロジェクトのスコープに収まるようにして、筋の良い実装も用意して、自分の目から見たら「プロジェクトに馴染む」としか思えない、という状態にまで練り上げたとしても、要望・変更を受け入れてもらえない場合はあります。最終的に要望を受け入れるかどうかは、そのプロジェクト内部の人が決定することです。外部から協力するだけの立場の人には、残念ですがその判断への介入はできません。
