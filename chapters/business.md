...
[^non-oss]: この結果MongoDBが採用した「Server Side Public License（SSPL）」や、RedisやKafkaから分離された一部のモジュールのライセンスは、オープンソースライセンスではないため、これらは「OSS」ではなくなっています（RedisやKafkaのコアモジュールは依然としてOSSです）。
