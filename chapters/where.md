...
[^mailing-list]: 参加者が「投稿先用のメールアドレス」宛にメールを送ると、参加者全員にそのメールが配信される、メールベースで掲示板のようなことを実現したシステム。歴史の長いOSSプロジェクトで使われていることが多い。
...
フィードバックを行う最初の時点では、皆さんはきっと「失敗しないだろうか？」とビクビクしているでしょうから、これらのことを意識する必要はあまり無いかもしれません。それよりも、気をつけないといけない度合いが高いのは、*フィードバックに対する開発者の反応が芳しくないものだった場合や、フィードバックを何度も重ねて「常連」になり、他の報告者に何かアドバイスをしようとした場合など*です。これについては、第3部「フィードバックした後のこと」で詳しく述べることにします。
