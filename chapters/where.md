...
フィードバック先窓口が分からない場合は、まずそのOSSのプロジェクト公式Webサイトを確認してみましょう。OSSによっては、*要望や不具合のフィードバック方法を説明している*場合があります。「コントリビュート（Contribution）」や「開発に参加するには（Get Involved）」といったリンクがあれば、まずはそこを見てみるとよいでしょう。

たとえば、全文検索エンジンの[Groonga](https://groonga.org/ja/)では、公式サイト上の「開発者向け情報」というリンクを辿った先に[バグレポートの送り方の説明ページ](https://groonga.org/ja/docs/contribution/report.html)があり、GitHubのイシュートラッカーかメーリングリスト[^mailing-list]で報告する旨の案内が書かれています。
...
小規模なプロジェクトでは、公式サイトに案内が無かったり、そもそも、RubyGemsやアプリストアのようなWebサイト上のダウンロードページしか無かったりもします。その場合、先の例のような方法で連絡を取れないか調べてみるしかありません。
...
ソフトウェア開発の現場にも、不具合の修正状況を把握・管理するために同様のシステムがあり、「ソフトウェアの不具合」を「バグ」と呼ぶことから、*「バグトラッキングシステム（Bug Tracking System、以下BTS）」*と呼ばれます。
...
![GitHubでの新規イシュー編集の様子](images/where-issue.png)

このページのイシュー一覧の右上に*「New issue」*というボタンがあり、クリックすると新しいイシューの入力画面に遷移します（プロジェクトによっては、イシューの種類の選択画面が挟まることもあります）。タイトルと説明を入力して「Submit new issue」ボタンを押せば、それでもうイシュー登録は完了です。
...
開発者視点で言うと、*1対1のコミュニケーション方法は、開発者側の負担が大きいので辛い*、というのが正直なところです。イシュートラッカーでなら他の詳しい人に回答してもらえる内容でも、メールだと、受け取った開発者が自分で返事をしないといけません。
...
大抵の場合、フォーラムはユーザーコミュニティ、イシュートラッカーは開発者コミュニティが使っています。メーリングリストは、ユーザー向けと開発者向けの2つが同時に存在する場合もあります。どちらかにしか関わっていない人もいれば、一人で両方に関わっている人もいます。
...
OSS Gateワークショップでも、「こんな簡単な・くだらないレベルのことを開発者に報告してもいいの？」「フォーラムとイシュートラッカーのどっちに投稿すればいいの？」と質問されることはたびたびあります。

また、そもそも「どのプロジェクト（開発元）に言えばいいのか？」という質問もよくあります。複数のソフトウェアが関わる問題では、経験者でも、どのソフトウェアが問題の主要因なのか判断が付かないことがあります。

　

結論を先に述べると、これらの問いに対する筆者の回答は以下のようになります。
...
ここで参考事例として、筆者が2019年に行ったフィードバック[^feedback-from-client]をご紹介します。
...
この現象から思いつくキーワードでWebを検索してみると、Firefoxのユーザーコミュニティが運営している「MozillaZine.jp」というサイトのフォーラムに、[約半年ほど前の時点で同様の事例の投稿があった](https://forums.mozillazine.jp/viewtopic.php?f=3&t=17326)と分かりました。見てみると、スレッドの大まかな流れは以下のようになっていました。

1. このような現象が起こっている、既知の解決策はどれも有効でなかった、という情報が投稿される。
2. 現象の再現条件の特定が複数人によって進められる。
3. Thunderbirdに対して行われた変更のうち、どの変更以降からこの現象が起こるようになったのかが特定される。
4. 暫定的な回避作として、とあるアドオンをインストールすると現象が起こらなくなる、という情報が寄せられる。（その後半年間、動きは無し）

次に、Mozillaが運用しているイシュートラッカーの[Bugzilla](https://bugzilla.mozilla.org)にすでに報告されている問題かどうかを調べました。しかしながら、思いつく限りのキーワードで検索しても該当するBugを見つけられなかったため、[新しいBugとして報告することにしました](https://bugzilla.mozilla.org/show_bug.cgi?id=1550670)。前章で述べたような「傍観者心理」によって誰も報告しないままになるといけないので、筆者はこういう場合は重複を恐れずに報告することにしています。もしすでに報告されている問題があれば、そちらへ誘導してもらえるはずと考えたからです。
...
仮にこれがGeckoの明白なミスに起因する現象であれば、Gecko側で修正されてBugはクローズされます。しかし実際には、Mozillaの担当者の方による詳細な調査ではGecko側に問題は見当たらず、Microsoft IME側の不具合である可能性があるとして、その方からMicrosoftのフィードバック窓口へ改めて報告されたそうです。
...
そう考えれば、そこで行われている議論や開発に文字通り「誰でも」気軽についていけるわけではなく、「開発者として」関わる必要があるのも納得しやすいでしょう。
...
[^non-software-engineer]: 開発ではなく運用に従事している人や、まだ就職していなくて学生で開発に参加する人もいます。
...
フィードバックを行う最初の時点では、皆さんはきっと失敗のほうを恐れて慎重になっているでしょうから、これらのことを意識する必要はあまり無いかもしれません。
